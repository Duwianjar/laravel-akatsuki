...
<section id="services " class="services" style="background-color: #24395d !important; ;">
    <div class="container pt-5" data-aos="fade-up">
            ...
            <div id="mapel" class="mapel col-md-12 p-4 bg-white rounded rounded-3 head shd-blue">
                <p class="px-2 ket fs-7 " >
                    ...
                    <form role="form" method="post" action="{{ url('/users/edit/index') }}">
                        @csrf
                        <input type="number" name="id" value="{{ $users->id }}" hidden/>
                        <div class="judul-edit">
                            <p>Nama Lengkap</p>
                            <input class="namamapel" type="text" class="form-control" name="namalengkap" value="{{ $users->fullname }}"/>
                        </div>
                        <div class="judul-edit">
                            <p>Telepon</p>
                            <input class="namamapel" type="number" class="form-control" name="telepon" value="{{ $users->telepon }}"/>
                        </div>
                        <div class="judul-edit">
                            <p>Email</p>
                            <input class="namamapel" type="email" class="form-control" name="email" value="{{ $users->email }}"/>
                        </div>
                        <div class="judul-edit">
                            @php
                                $decryptedpass = Crypt::decrypt($users->password);
                            @endphp
                            <p>Password</p>
                            <input class="namamapel" type="text" class="form-control" name="password" value="{{ $decryptedpass }}"/>
                        </div>
                        <div class="judul-edit">
                            <p>Role</p>
    ...
    @if ($users->role)
    <script>
        var selectedValue = {!! json_encode($users->role) !!}; // Escape dan konversi nilai
    ...
    @if ($users->id_kelas)
    <script>
        var selectedValue = {!! json_encode($users->id_kelas) !!}; // Escape dan konversi nilai
