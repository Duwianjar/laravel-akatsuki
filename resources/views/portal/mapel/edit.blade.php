...
<section id="services " class="services" style="background-color: #24395d !important; ;">
    <div class="container pt-5" data-aos="fade-up">
            ...
            <div id="mapel" class="mapel col-md-12 p-4 bg-white rounded rounded-3 head shd-blue">
                <p class="px-2 ket fs-7 " >
                    ...
                    <form role="form" method="post" action="{{ url('/mapel/edit/index') }}">
                        @csrf
                        <input type="number" name="id" value="{{ $mapel->id }}" hidden/>
                        <div class="judul-edit">
                            <p>Nama Mapel</p>
                            <input class="namamapel" type="text" class="form-control" name="namamapel" value="{{ $mapel->namamapel }}"/>
                        </div>
                        ...
                        <div class="judul-edit">
                            <p>Deskripsi</p>
                            <textarea class="deskripsi" name="deskripsi">{{ $mapel->deskripsi }}</textarea>
                        </div>
                        <div class="judul-edit">
    ...
    @if ($gurusekarang->id)
    <script>
        var selectedValue = {!! json_encode($gurusekarang->id) !!}; // Escape dan konversi nilai
        var selectElement = document.getElementById('pilihan_guru');
    ...
    @if ($kelassekarang->id)
    <script>
        var selectedValue = {!! json_encode($kelassekarang->id) !!}; // Escape dan konversi nilai
